...
select
    *,
    ...
    case
        ...
        when "מרכיבי אומדן הכנסות" = 'שירותי קבלן ראשי' then 4
        when "מרכיבי אומדן הכנסות" = 'עבודות נוספות וחריגים' then 5
        when "מרכיבי אומדן הכנסות" = 'קיזוז מזמין מוסכם' then 6
        when "מרכיבי אומדן הכנסות" = 'קיזוזי מזמין' then 7
        when "מרכיבי אומדן הכנסות" = 'מנהל מודל BIM (50%)' then 8
        when "מרכיבי אומדן הכנסות" = 'התייקרות בפועל' then 9
        when "מרכיבי אומדן הכנסות" = 'התייקרות על היתרה (מדד ידוע)' then 10
        when "מרכיבי אומדן הכנסות" = 'קיזוז ביטוח (0.75%)' then 11
        else 99 end as "סדר מיון"
from final_result